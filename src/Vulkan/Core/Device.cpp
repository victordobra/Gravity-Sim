...
namespace gsim {
	...
	static const std::vector<const char*> MANDATORY_DEVICE_EXTENSIONS = {
	...
	static VulkanQueueFamilyIndices GetQueueFamilies(VkPhysicalDevice physicalDevice) {
		...
		if(!queueFamilies)
			GSIM_LOG_FATAL("Failed to allocate queue family array!");
	...
	static uint32_t CalculateDeviceScore(VkPhysicalDevice physicalDevice) {
		...
		if(!extensions)
			GSIM_LOG_FATAL("Failed to allocate device extension array!");
		...
		for(VkExtensionProperties* extension = extensions; extension != extensionsEnd; ++extension) {
			...
			for(const char* mandatoryExtension : MANDATORY_DEVICE_EXTENSIONS)
			...
			for(const char* optionalExtension : OPTIONAL_DEVICE_EXTENSIONS)
	...
	static bool FindPhysicalDevice() {
		...
		if(!physicalDevices)
			GSIM_LOG_FATAL("Failed to allocate physical device array!");
	...
	static void SetupPhysicalDevice() {
		...
		VkExtensionProperties* extensions = (VkExtensionProperties*)malloc(extensionCount * sizeof(VkExtensionProperties));
		if(!extensions)
			GSIM_LOG_FATAL("Failed to allocate physical device extension array!");
		...
		for(VkExtensionProperties* extension = extensions; extension != extensionsEnd; ++extension) {
			...
			for(const char* mandatoryExtension : MANDATORY_DEVICE_EXTENSIONS)
				if(!strncmp(extension->extensionName, mandatoryExtension, VK_MAX_EXTENSION_NAME_SIZE)) {
					// Add the current extension to the array
					deviceExtensions.push_back(mandatoryExtension);
		...
		GSIM_LOG_INFO("Picked Vulkan physical device: %s", deviceProperties.deviceName);
		GSIM_LOG_INFO("Vulkan physical device queue families: graphics: %u; present: %u; transfer: %u; compute: %u", queueFamilies.graphicsQueueIndex, queueFamilies.presentQueueIndex, queueFamilies.transferQueueIndex, queueFamilies.computeQueueIndex);
	}
	static void CreateLogicalDevice() {
		...
		VkQueueFamilyProperties* queueFamilyProperties = (VkQueueFamilyProperties*)malloc(queueFamilyCount * sizeof(VkQueueFamilyProperties) + queueFamilyCount * sizeof(uint32_t));
		if(!queueFamilyProperties)
			GSIM_LOG_INFO("Failed to allocate queue family array!");
		...
		float queuePriotities[4]{ 1.f, 1.f, 1.f, 1.f };
		...
		if(result != VK_SUCCESS)
			GSIM_LOG_FATAL("Failed to create Vulkan logical device! Error code: ", string_VkResult(result));
		...
		GSIM_LOG_INFO("Create Vulkan logical device with %u unique queues.", uniqueQueueCount);
	}
