...
namespace gsim {
	...
	void CreateVulkanCommandPools() {
		...
		if(result != VK_SUCCESS)
			GSIM_LOG_FATAL("Failed to create Vulkan graphics command pool! Error code: %s", string_VkResult(result));
		...
		if(queueFamilies.transferQueueIndex == queueFamilies.graphicsQueueIndex)
			...
			if(result != VK_SUCCESS)
				GSIM_LOG_FATAL("Failed to create Vulkan transfer command pool! Error code: %s", string_VkResult(result));
		...
		if(queueFamilies.computeQueueIndex == queueFamilies.graphicsQueueIndex)
			...
			if(result != VK_SUCCESS)
				GSIM_LOG_FATAL("Failed to create Vulkan present command pool! Error code: %s", string_VkResult(result));
			
			// Increment the unique command pool count
			++uniqueCommandPoolCount;
		}

		GSIM_LOG_INFO("Created %u unique Vulkan command pools.", uniqueCommandPoolCount);
	}
